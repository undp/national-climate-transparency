...
import { CompanyRole, Programme, addCommSep } from '@undp/carbon-library';
...
const ProgrammeTransferForm: FC<ProgrammeTransferFormProps> = (
    ...
    if (newValue !== undefined) {
      ...
      setCompanyList(
        resp.data
          ...
          .filter((d: any) => {
            return d.value !== userCompanyId && String(d.state) === CompanyState.ACTIVE.valueOf();
          })
      );
    } else {
  ...
  for (const index in programme.creditOwnerPercentage) {
    if (
      ...
      String(companies[Number(programme.companyId[index])].state) === CompanyState.ACTIVE.valueOf()
    ) {
