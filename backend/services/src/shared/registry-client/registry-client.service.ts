...
export class RegistryClientService {
  ...
  public async createProgramme(programme: ProgrammeDto) {
    if (!this.configService.get("registry.syncEnable")) {
      this.logger.debug("Programme created ignored due to registry sync disable");
      return;
    }
    ...
    if (programme.ndcAction) {
        ...
        if (programme.ndcAction.agricultureProperties) {
            programmeReq["mitigationActions"].push(programme.ndcAction.agricultureProperties)
        } else if (programme.ndcAction.solarProperties) {
            programmeReq["mitigationActions"].push(programme.ndcAction.solarProperties)
        } else {
            programmeReq["mitigationActions"].push({})
        }

        programmeReq["mitigationActions"][0]["typeOfMitigation"] = programme.ndcAction.typeOfMitigation;
        programmeReq["mitigationActions"][0]["userEstimatedCredits"] = programme.ndcAction.ndcFinancing.userEstimatedCredits;
        programmeReq["mitigationActions"][0]["systemEstimatedCredits"] = programme.ndcAction.ndcFinancing.systemEstimatedCredits;
    }
    return await axios.post(
      this.configService.get("registry.endpoint") + "/national/programme/create",
      programmeReq,
      {
        headers: {
          api_key: `${this.configService.get("registry.apiToken")}`,
