...
WITH fullprj AS (
	SELECT 
			...
			p_v_e."internationalImplementingEntities",
...
act AS (
	SELECT 
			...
			CUSTOM_ARRAY_AGG(a."internationalImplementingEntity") FILTER (WHERE a."internationalImplementingEntity" IS NOT NULL)::character varying[] AS "internationalImplementingEntities"
	FROM 
			activity a
	LEFT JOIN (
        SELECT 
...
SELECT 
	...
	CUSTOM_ARRAY_AGG(DISTINCT COALESCE(fullprj."internationalImplementingEntities", '{}') || COALESCE(act."internationalImplementingEntities", '{}')) FILTER (WHERE (fullprj."internationalImplementingEntities" IS NOT NULL OR act."internationalImplementingEntities" IS NOT NULL)) AS "internationalImplementingEntities",
