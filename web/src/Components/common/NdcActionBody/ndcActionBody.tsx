...
import { isValidateFileType } from '../../../Utils/documentValidators';
import { linkDocVisible, uploadDocUserPermission } from '../../../Utils/documentPermissions';
import { useConnection } from '../../../Context/ConnectionContext/connectionContext';
import { useUserContext } from '../../../Context/UserInformationContext/userInformationContext';
import { RejectDocumentationConfirmationModel } from '../RejectionModel.tsx/RejectDocConfirmModel';
      ...
      if (isValidateFileType(file?.type)) {
        ...
        if (response?.data) {
          ...
          if (
            ...
            const programmeData = response?.data.programme;
            let modified = false;
            programmeData.mitigationActions.map((action: any) => {
              if (action.actionId === ndcActionId) {
                modified = true;
                let docAdded = false;
                for (const documentDetails of action.projectMaterial) {
                  let document: any;
  ...
  const docAction = async (id: any, status: DocumentStatus, actionId: any, type: any) => {
    ...
    try {
      ...
      if (type === DocType.VERIFICATION_REPORT && status === DocumentStatus.ACCEPTED) {
        const programmeRes = await post('national/programme/query', {
          page: 1,
          size: 2,
          filterAnd: [
            {
        ...
        const programmeData = programmeRes.data[0];
